...
- **상담 정보 체크리스트**: 견적에 필요한 정보 중 확인된 것과 부족한 것을 표시하고, 부족한 정보를 묻는 추천 질문을 자동 생성해 답변에 한 번에 추가
...
- 참고 이미지/도안 첨부 (타투 상담의 핵심 산출물 — 최우선 후보)
...
npm run test:e2e    # API 기반 E2E smoke test (24 checks)
```
...
- **세션 토큰화** — 현재 세션은 서명 없는 쿠키에 사용자 ID를 그대로 담고, `x-replydesk-user-id` 헤더 폴백도 남아 있어 위조 가능. 랜덤 세션 토큰 + 서버 검증으로 교체 필요
- **`/api/generate-reply` 인증 게이팅** — 이 라우트는 세션/워크스페이스 검증 없이 호출 가능. 다른 데이터 라우트와 동일하게 게이팅 필요 (OpenAI 비용 남용 방지)
- **데모 백도어 제거** — 비밀번호 미설정 계정의 `admin1234` 레거시 로그인은 데모 전용
